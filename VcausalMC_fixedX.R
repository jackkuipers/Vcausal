...
xi <- 1/3 # fixed fraction of node X
...
for (ii in 1:nreps) {
  ...
  binary_sample <- c(sample(1:4, N0, replace = TRUE, prob = c(p0, p1, p2, p3)), 
                     sample(5:8, N1, replace = TRUE, prob = c(p4, p5, p6, p7)))
...
source("Vcausalfns_fixedX.R")
# theoretical sd of causal estimators 
opt <- find_alpha_fixedX(pZ, pY, c(N0, N1))
print(sqrt(opt$VR))
print(sqrt(opt$VM))
print(sqrt(opt$CMR))
